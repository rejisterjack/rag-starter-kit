...
function MessageCircleIcon({ size = 24, color = 'currentColor' }: { size?: number; color?: string }) {
  return (
...
export function RAGChatWidget({
    ...
    await streamChatResponse(
      ...
      (err: Error) => {
        ...
        setMessages((prev) => {
          ...
          return prev.map((m) =>
            m.id === assistantId ? { ...m, content: m.content || 'Sorry, something went wrong.' } : m
          );
        });
        abortRef.current = null;
      },
  ...
  }, [input, isLoading, apiKey, baseUrl, buildHistory]);
        ...
        <div style={styles.panel}>
                ...
                <div style={msg.role === 'user' ? styles.bubble_user : styles.bubble_assistant}>
                  {msg.content || (
                    <div style={styles.typingIndicator}>
                      <div style={{ ...styles.typingDot, animation: 'rag-dot-bounce 1.4s infinite ease-in-out both' }} />
                      <div style={{ ...styles.typingDot, animation: 'rag-dot-bounce 1.4s infinite ease-in-out 0.16s both' }} />
                      <div style={{ ...styles.typingDot, animation: 'rag-dot-bounce 1.4s infinite ease-in-out 0.32s both' }} />
                    </div>
                  )}
                </div>
                {msg.role === 'user' && (
          ...
          <div style={styles.inputArea}>
            ...
            <button
              ...
              onMouseLeave={(e) => {
                (e.currentTarget as HTMLElement).style.backgroundColor = isLoading ? colors.textMuted : colors.buttonBg;
              }}
            >
              <SendIcon size={18} />
            </button>
