...
function parseSlide(
  ...
  const text = shapes.map((s) => s.paragraphs.map((p) => p.text).join('\n')).join('\n\n');
    ...
    for (const shape of slide.shapes) {
      if (shape.paragraphs.length > 0) {
        ...
        for (const paragraph of shape.paragraphs) {
          ...
          const bullet =
            shape.type === 'body' && paragraph.level !== undefined
...
export function getOutline(parsed: ParsedPPTX): string[] {
  return parsed.slides.map((s) => s.title || `Slide ${s.number}`).filter(Boolean) as string[];
}
...
export function isValidPPTX(buffer: Buffer): boolean {
  try {
    ...
    logger.debug('PPTX validation failed', {
