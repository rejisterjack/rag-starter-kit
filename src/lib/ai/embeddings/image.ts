...
async function loadCLIPModel() {
  ...
  try {
    ...
    console.log('[ImageEmbeddings] CLIP model loaded successfully');
    return { model: clipModel, processor: clipProcessor };
  } catch (error) {
    console.error('[ImageEmbeddings] Failed to load CLIP model:', error);
...
async function getCachedEmbedding(cacheKey: string): Promise<number[] | null> {
  ...
  try {
    ...
    // Database cache not available, continue without it
    console.warn('[ImageEmbeddings] Database cache error:', error);
  }
 ...
 */
async function cacheEmbedding(cacheKey: string, embedding: number[], _imageUrl?: string): Promise<void> {
  // Update in-memory cache
  embeddingCache.set(cacheKey, { embedding, timestamp: Date.now() });
...
export async function generateImageEmbedding(imageBuffer: Buffer | string): Promise<number[]> {
  ...
  try {
    ...
    console.error('[ImageEmbeddings] Failed to generate embedding:', error);
 ...
 */
export async function generateImageEmbeddings(imageBuffers: (Buffer | string)[]): Promise<number[][]> {
    ...
    const batchEmbeddings = await Promise.all(
      batch.map(async (buffer) => {
        try {
          ...
          console.error('[ImageEmbeddings] Batch processing error:', error);
          return null;
        }
      })
    );
...
export async function generateTextEmbeddingForImageSearch(text: string): Promise<number[]> {
  ...
  try {
    ...
    console.error('[ImageEmbeddings] Failed to generate text embedding:', error);
