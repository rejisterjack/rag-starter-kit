 ...
 * Implementation of the LLMProvider interface using Anthropic API via Vercel AI SDK
 */
...
export class AnthropicProvider implements LLMProvider {
  ...
  constructor(config: AnthropicConfig = { provider: 'anthropic' }) {
    ...
    this.defaultModel = config.defaultModel ?? ANTHROPIC_MODELS.CLAUDE_SONNET_4;
  ...
  private handleError(error: unknown, model: string): LLMError {
    ...
    // Handle rate limiting (429) and Anthropic overloaded (529)
    if (statusCode === 429 || statusCode === 529 || message.toLowerCase().includes('rate limit')) {
    ...
    // Handle context length exceeded (400)
