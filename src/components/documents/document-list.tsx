...
export function DocumentList({
  ...
  return (
    <div className={cn('glass flex h-full flex-col border-r border-border/50 bg-background/60 shadow-xl backdrop-blur-xl', className)}>
      {/* Header */}
        ...
        <div className="mt-4 flex items-center justify-between">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="gap-2 h-9 rounded-lg bg-transparent border-foreground/10 hover:bg-foreground/5 transition-colors">
                <Filter className="h-4 w-4" />
                Filter
                {statusFilter.length > 0 && (
                  <Badge variant="secondary" className="ml-1 h-5 px-1.5 rounded-sm bg-primary/20 text-primary">
                    {statusFilter.length}
                  </Badge>
                )}
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="start" className="w-52 rounded-xl glass border-border/50 shadow-2xl">
              <DropdownMenuCheckboxItem checked={statusFilter.includes('completed')} onCheckedChange={() => toggleStatusFilter('completed')}>
              ...
              <DropdownMenuCheckboxItem checked={statusFilter.includes('error')} onCheckedChange={() => toggleStatusFilter('error')}>
          ...
          <div className="flex items-center gap-2">
            {documents.length > 0 && onDeleteAll && (
              <Button variant="ghost" size="icon" className="h-9 w-9 rounded-lg text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition-colors" onClick={onDeleteAll}>
                <Trash2 className="h-4 w-4" />
              </Button>
            )}
            <Button size="sm" className="gap-2 h-9 rounded-lg shadow-md shadow-primary/20" onClick={onUpload}>
      ...
      <ScrollArea className="flex-1 scrollbar-thin">
        <div className="p-4 space-y-3">
          {isLoading ? (
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground">
              ...
              {!searchQuery && statusFilter.length === 0 && onUpload && (
                <Button variant="outline" size="sm" className="mt-5 rounded-lg border-primary/20 bg-primary/5 text-primary hover:bg-primary/10 transition-colors" onClick={onUpload}>
