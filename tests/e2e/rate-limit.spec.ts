...
test.describe('Chat Rate Limiting', () => {
  ...
  test('should track rate limits per user', async ({ page, request }) => {
    ...
    const user1Responses = await makeRequests(
      request,
      '/api/chat',
      10,
      'POST',
    ...
    const user2Responses = await makeRequests(
      request,
      '/api/chat',
      5,
      'POST',
...
test.describe('Authentication Rate Limiting', () => {
  test('should enforce login rate limit', async ({ page }) => {
    ...
    if (response.status() === 429) {
      ...
      await expect(page.locator('text=too many|rate limit|try again later', { 
        ignoreCase: true 
      })).toBeVisible();
    }
  });
